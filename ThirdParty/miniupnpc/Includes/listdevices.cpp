...
void add_device(struct upnp_dev_list * * list_head, struct UPNPDev * dev)
{
	...
	for(elt = *list_head; elt != NULL; elt = elt->next) {
		if(strcmp(elt->descURL, dev->descURL) == 0) {
			...
			if(elt->count >= elt->allocated_count) {
				...
				tmp =(UPNPDev * *) realloc(elt->array, elt->allocated_count * sizeof(struct UPNPDev *));
				if(tmp == NULL) {
	...
	elt = (upnp_dev_list*)malloc(sizeof(struct upnp_dev_list));
	if(elt == NULL) {
		fprintf(stderr, "Failed to malloc(%lu)\n", (unsigned long)sizeof(struct upnp_dev_list));
		return;
	}
	elt->next = *list_head;
	elt->descURL = _strdup(dev->descURL);
	if(elt->descURL == NULL) {
	...
	elt->allocated_count = ADD_DEVICE_COUNT_STEP;
	elt->array =(UPNPDev **) malloc(ADD_DEVICE_COUNT_STEP * sizeof(struct UPNPDev *));
	if(elt->array == NULL) {
